...
/* ---------------------------------------------------------------------------------- */
/*  OpenCart Factory (with support for the Override Engine)                           */
/*                                                                                    */
...
final class Factory 
{
	...
	public function modCheck( $file ) {
		// return a PHP file possibly modified by vqmod (see also http://code.google.com/p/vqmod/)
		if (!empty($GLOBALS['vqmod'])) {
			// vqmod version 2.3.2 or smaller
	...
	private function newInstance( $type, $file, $class, $args=array() ) {
		...
		require_once( $this->modCheck( $file ) );
		...
		foreach ($modFiles as $addon => $modFile) {
			if ($parent == $class) {
				...
				$GLOBALS[$var_id] = $this->classWithModifiedParentName( $modFile, $parent );
				require_once( "var://".$var_id );
			}
			$child = str_replace('-','_',$addon).'_'.$class;
			// only 1 child class allowed if it is final
	...
	public function readTemplate( $template ) {
		...
		if (file_exists( $file ) && is_file( $file )) {
			return file_get_contents( $this->modCheck( $file ) );
		} else {
	...
	public function newMail() {
		return $this->newSystemClass( 'library/mail.php' );
	}
	...
	public function newTemplate() {
		return $this->newSystemClass( 'library/template.php' );
	}
	...
	public function newOpenbay( $registry ) {
		return $this->newSystemClass( 'library/openbay.php', array( $registry ) );
	}


	public function newPlay( $registry ) {
		return $this->newSystemClass( 'library/play.php', array( $registry ) );
	}


	public function newEbay( $registry ) {
		return $this->newSystemClass( 'library/ebay.php', array( $registry ) );
	}


	public function newAmazon( $registry ) {
		return $this->newSystemClass( 'library/amazon.php', array( $registry ) );
	}


	public function newAmazonus( $registry ) {
		return $this->newSystemClass( 'library/amazonus.php', array( $registry ) );
	}
}
?>